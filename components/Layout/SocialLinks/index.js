...
const SocialLinks = memo((props) => {
  ...
  return (
    <ul className="list-reset flex justify-center sm:justify-start items-center">
      {socials.map((e, index) => (
        <li className={`${index === 0 ? "ml-20" : ""} ${index < socials.length ? "mr-10" : ""}`} key={index}>
          <a
            title={e.label}
            className={`relative overflow-hidden text-blue border-2 border-solid border-blue rounded-full flex items-center justify-center h-30 w-30`.concat(
              " ",
              css({
