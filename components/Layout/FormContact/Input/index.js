...
const Input = (props) => {
  ...
  return (
    <Fragment>
      <label
        className={`${themeSystem.display[tagType]} text-blue text-15 sm:text-25 text-left whitespace-no-wrap`}
        htmlFor={name}
      >
        {label}
        <div
          className={`relative ${tagType === "textarea" ? "" : "md:w-6/12 flex-grow"} ${styles.wrapper}`.concat(
            " ",
            css({
