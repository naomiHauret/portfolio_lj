...
class About extends PureComponent {
  ...
  _renderSection = (content, availablesLocales, locale, dataId, galleryId) => {
    return <section className="mt-50 md:mt-70 flex flex-col md:flex-row">
      ...
      <ul className="list-reset flex-grow flex flex-wrap mt-10 md:mt-0">
        {content[`${dataId}-gallery`].map((picture, key) => <li
  ...
  render() {
    ...
    return <Layout seo={seo} locale={locale}>
      {this.props.error ? <Fragment /> : <Fragment>
        <div className="md:px-1/12  mb-40 sm:mb-70">
          <section className="flex flex-col-reverse md:flex-row">
              ...
              <div className="w-full md:w-4/12 md:ml-60 text-center md:text-left">
                <img className="rounded-full overflow-hidden text-center md:text-left" src={portraits[Math.floor(Math.random() * portraits.length)]} />
              </div>
            </section>
