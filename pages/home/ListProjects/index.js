...
const ListProjects = memo(
  (props) => {
    ...
    return <ul className={`list-reset mx-auto ${styles.projectsGrid}`}>
      {projects.map((project, k) => (
        <li className="md:cursor-pointer overflow-hidden relative" key={project.uid}>
          <animated.div className='absolute pin-t pin-l bg-violet h-full w-full z-10' style={useSpring({
            to: [{ transform: 'translateX(0)' }, { transform: 'translateX(100%)'}], from: { transform: 'translateX(100%)', width: "100%" },
          ...
          >
            ...
            >
              <animated.figure
                style={
                  useSpring({
                    to: { opacity: 1, transform: 'translateX(0)' }, from: { opacity: 0,  transform: 'translateX(-100%)' },
              ...
              >
                <div className="p-40 h-full sm:p-30 m-0 flex items-center justify-center">
                  <img
                    ...
                    src={project["image-preview"]["menu-preview"].url}
                  />
                </div>
      ...
      <li >
        <aside className="sm:ml-20 ">
          <animated.h2 style={
            useSpring({
              to: { opacity: 1, transform: 'translateX(0)' }, from: { opacity: 0, transform: 'translateX(-10px)' },
          ...
          <animated.div className={`font-500 text-gray leading-30 text-14`}
            style={
              useSpring({
